...
def generate_swap_suggestions(meal_items: list[dict], breaches: dict, profile: dict) -> list[str]:
    """LLM-driven culturally relevant swap suggestions."""
    if not breaches:
        return []
...
def node(state: GlucoLensState) -> dict:
    ...
    try:
        ...
        recommendations = (
            generate_swap_suggestions(state.get("meal_items", []), flagged, profile)
            if flagged else []
        )
